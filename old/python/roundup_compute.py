...
def completeComputationLoadGenomes(computeDir, genomes):
    ...
    for genome in genomes:
        ...
        if getKVCacheValue(computeDir, key):
            continue
        startTime = time.time()
        fastaPath = roundup_common.fastaFileForDbPath(roundup_common.makeDbPath(genome, dir=getGenomesDirFromComputeDir(computeDir)))
        ids = list(fasta.readIds(fastaPath))
