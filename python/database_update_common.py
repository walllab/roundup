...
def withUpdateDbConn(conn=None, commit=True, host=DEV_MYSQL_HOST):
    '''
    get a connection to the database server.  currently returns conn to the prod server, whether the code is run from prod or dev environments.
    '''
    if conn == None:
        host = os.environ.get(rodeo_db.RODEO_MYSQL_SERVER, host)
        for conn in rodeo_db.withConnFromAnywhere(commit=commit, host=host, db="go"):
            yield conn
    else:
        yield conn
