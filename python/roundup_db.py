...
def dropRoundupResultsTable(conn=None, commit=True):
    sql = 'DROP TABLE IF EXISTS '+ROUNDUP_MYSQL_DB+'.roundup_results'
    for conn in withRoundupDbConn(conn=conn):
        rodeo_db.executeSQL(sql, conn=conn, commit=commit)


def createRoundupResultsTable(conn=None, commit=True):
...
def dropRoundupSequenceTable(conn=None, commit=True):
    sql = '''DROP TABLE IF EXISTS '''+ROUNDUP_MYSQL_DB+'''.roundup_sequence'''
    for conn in withRoundupDbConn(conn=conn):
        rodeo_db.executeSQL(sql, conn=conn, commit=commit)


def createRoundupSequenceTable(conn=None, commit=True):
...
def dropRoundupSequenceToGoTermTable(conn=None, commit=True):
    sql = '''DROP TABLE IF EXISTS '''+ROUNDUP_MYSQL_DB+'''.roundup_sequence_to_go_term'''
    for conn in withRoundupDbConn(conn=conn):
        rodeo_db.executeSQL(sql, conn=conn, commit=commit)


def createRoundupSequenceToGoTermTable(conn=None, commit=True):
...
def dropRoundupGenomesTable(conn=None, commit=True):
    sql = '''DROP TABLE IF EXISTS '''+ROUNDUP_MYSQL_DB+'''.roundup_genomes'''
    for conn in withRoundupDbConn(conn=conn):
        rodeo_db.executeSQL(sql, conn=conn, commit=commit)


def createRoundupGenomesTable(conn=None, commit=True):
...
def dropRoundupDivergencesTable(conn=None, commit=True):
    sql = '''DROP TABLE IF EXISTS '''+ROUNDUP_MYSQL_DB+'''.roundup_divergences'''
    for conn in withRoundupDbConn(conn=conn):
        rodeo_db.executeSQL(sql, conn=conn, commit=commit)


def createRoundupDivergencesTable(conn=None, commit=True):
...
def dropRoundupEvaluesTable(conn=None, commit=True):
    sql = '''DROP TABLE IF EXISTS '''+ROUNDUP_MYSQL_DB+'''.roundup_evalues'''
    for conn in withRoundupDbConn(conn=conn):
        rodeo_db.executeSQL(sql, conn=conn, commit=commit)


def createRoundupEvaluesTable(conn=None, commit=True):
...
def updateRoundupSequenceToGoTermTable(conn=None, commit=True):
...
def updateRoundupSequenceTable(conn=None, commit=True):
    '''
...
def getDatabaseForId(id, conn=None):
    ...
    '''
    for conn in withRoundupDbConn(conn=conn):
        return rodeo_db.getValueForId(id=id, conn=conn, tableName='roundup_genomes', dbName=ROUNDUP_MYSQL_DB, valueCol='name')
...
def getDivergenceForId(id, conn=None):
    ...
    '''
    for conn in withRoundupDbConn(conn=conn):
        return rodeo_db.getValueForId(id=id, conn=conn, tableName='roundup_divergences', dbName=ROUNDUP_MYSQL_DB, valueCol='name')
...
def getEvalueForId(id, conn=None):
    ...
    '''
    for conn in withRoundupDbConn(conn=conn):
        return rodeo_db.getValueForId(id=id, conn=conn, tableName='roundup_evalues', dbName=ROUNDUP_MYSQL_DB, valueCol='name')
...
def getIdForDatabase(db, conn=None, insertMissing=False):
    ...
    '''
    for conn in withRoundupDbConn(conn=conn):
        return rodeo_db.getIdForValue(value=db, conn=conn, tableName='roundup_genomes',
                                      dbName=ROUNDUP_MYSQL_DB, valueCol='name', insertMissing=insertMissing)
...
def getIdForEvalue(evalue, conn=None, insertMissing=False):
    ...
    '''
    for conn in withRoundupDbConn(conn=conn):
        return rodeo_db.getIdForValue(value=evalue, conn=conn, tableName='roundup_evalues',
                                      dbName=ROUNDUP_MYSQL_DB, valueCol='name', insertMissing=insertMissing)
...
def getIdForDivergence(divergence, conn=None, insertMissing=False):
    ...
    '''
    for conn in withRoundupDbConn(conn=conn):
        return rodeo_db.getIdForValue(value=divergence, conn=conn, tableName='roundup_divergences',
                                      dbName=ROUNDUP_MYSQL_DB, valueCol='name', insertMissing=insertMissing)
...
def loadGenome(genome, ids):
    ...
    for conn in withRoundupDbConn():
        ...
        # externalSeqIdAndGenomeIdPairsList = [(seqId, genomeId) for seqId in ids]
        # rodeo_db.executeManySQL(sql, args=externalSeqIdAndGenomeIdPairsList, conn=conn)
...
def loadResultsFile(resultsFile, seqIdMap=None):
    ...
    for conn in withRoundupDbConn():
        ...
        args = externalSeqIdAndGenomeIdPairsList
        rows = rodeo_db.selectManySQL(sql=sql, args=args, conn=conn)
        seqIdMap.update(dict([data for data in zip(args,[row[0] for row in rows])]))
...
def deleteGenomeByName(genome, conn=None):
    ...
    for conn in withRoundupDbConn(conn=conn):
        ...
        print sql
        rodeo_db.executeSQL(sql, conn=conn, args=[dbId, dbId])
        ...
        print sql
        rodeo_db.executeSQL(sql, conn=conn, args=[dbId])
...
def getSequenceIdToSequenceDataMap(sequenceIds, conn=None):
    ...
    for group in util.groupsOfN(sequenceIds, 1000):
        ...
        def selectSequenceDataRows():
            return rodeo_db.selectSQL(sql=sql, conn=conn)
...
def getOrthologs(conn, qdb, sdb, divergence='0.2', evalue='1e-20'):
    '''
    ...
    rows = rodeo_db.selectSQL(sql=sql, args=args, conn=conn)
...
def numOrthologs(conn=None):
    ...
    for conn in withRoundupDbConn(conn=conn):
        ...
        rows = rodeo_db.selectSQL(sql=sql, conn=conn)
...
def isLoadedResultForParams(params, conn=None):
    for conn in withRoundupDbConn(conn=conn):
        ...
        sql += ' WHERE rr.query_db = %s AND rr.subject_db = %s AND rr.divergence = %s AND rr.evalue = %s '
        args = (qdbId, sdbId, divId, evalueId)
        results = rodeo_db.selectSQL(sql=sql, args=args, conn=conn, commit=False)
        if results:
            return True
        else:
            return False
...
def findGeneNamesLike(substring, searchType=CONTAINS_TYPE, conn=None):
    ...
    for conn in withRoundupDbConn(conn=conn):
        return [row[0] for row in rodeo_db.selectSQL(sql=sql, args=args, conn=conn)]
...
def findGeneNameGenomePairsLike(substring, searchType=CONTAINS_TYPE, conn=None):
    ...
    for conn in withRoundupDbConn(conn=conn):
        return [(row[0], row[1]) for row in rodeo_db.selectSQL(sql=sql, args=args, conn=conn)]
...
def getSeqIdsForGeneName(geneName, database=None, conn=None):
    ...
    for conn in withRoundupDbConn(conn=conn):
        ...
        # results are a tuple of tuples which containing an id.  convert to a list of ids.
        return [row[0] for row in rodeo_db.selectSQL(sql=sql, args=params, conn=conn)]
