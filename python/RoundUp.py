...
def makeGetSeqForId(genomeFastaPath):
    ...
    for (seqNameline, seq) in fasta.readFastaIter(fh, ignoreParseError=True):
        seqId = BioUtilities.convertNamelineToId(seqNameline)
        fastaMap[seqId] = seq
    fh.close()
...
def makeGetHitsOnTheFly(genomeIndexPath, evalue, workingDir='.'):
    ...
    def getHitsOnTheFly(seqname, seq):
        with nested.NestedTempDir(dir=workingDir, nesting=0) as tmpDir:
            ...
            util.writeToFile('{0}\n{1}\n'.format(BioUtilities.convertIdToNameline(seqname), seq), queryFastaPath)
...
def getGoodEvalueHits(seqName, seq, getHitsFunc, getSeqFunc, evalue):
    ...
    if hits:
        hitCount = 0
        for hit in hits:
            if hitCount >= roundup_common.MAX_GOOD_EVALUE_HITS:
                break
...
def getDistanceForAlignedSeqPair(seqName, alignedSeq, hitSeqName, alignedHitSeq, workPath):
    ...
    try:
        ...
        BioUtilities.runPaml_all(workPath)
        distance = BioUtilities.paml_GetDistance(workPath)
        if DEBUG:
...
def roundupSome(querySeqIds, queryFastaPath, subjectFastaPath, div, evalue, outFile, workingDir='.'):
    ...
    if querySeqIds is None:
        querySeqIds = BioUtilities.getIdsForFastaPath(queryFastaPath)
    ...
    with nested.NestedTempDir(dir=workingDir, nesting=0) as tmpDir:
        ...
        data = ''.join(['%s %s %s\n'%(ortholog[subjectIndex], ortholog[queryIndex], ortholog[distanceIndex]) for ortholog in orthologs])
        util.writeToFile(data, outFile)


def roundup(queryFastaPath, subjectFastaPath, divEvalueAndOutfileList, forwardhits, reversehits, workingDir='.'):
    '''
    ...
    with nested.NestedTempDir(dir=workingDir, nesting=0) as tmpDir:
        ...
        retval = roundupSub(queryFastaPath, subjectFastaPath, divEvalueAndOutfileList, getForwardHits, getReverseHits, tmpDir, genomeSwapOptimization)
            

def roundupSub(queryFastaPath, subjectFastaPath, divEvalueAndOutfileList, getForwardHits, getReverseHits, workingDir, genomeSwapOptimization=True):
    '''
    ...
    # if swapped query and subject genome, need to swap ids in orthologs when writing output.
    if isSwapped:
        queryIndex = 0
        subjectIndex = 1
        distanceIndex = 2
...
def roundupSubSub(querySeqIds, getQuerySeqFunc, getSubjectSeqFunc, divEvalues, getForwardHits, getReverseHits, workingDir):
    ...
    shutil.copy(roundup_common.MATRIX_PATH, workingDir)
    shutil.copy(roundup_common.CODEML_CONTROL_PATH, workingDir)

    divEvalueToOrthologs = dict(((div, evalue), list()) for div, evalue in divEvalues)
    maxEvalue = max(evalue for div, evalue in divEvalues)
    maxDiv = max(div for div, evalue in divEvalues)
    ...
    for queryName in querySeqIds:
        ...
        for hitData in hitDataList:
            ...
            except roundup_common.RoundupException, e:
                continue
        ...
        for divEvalue in divEvalues:
            ...
            for hitData in distancesHitDataList:
                if hitData['hitEvalue'] < evalue and not hitData['tooDivergedPred'](div):
        ...
        for hitName in minimumHitNameToHitData:
            ...
            maxHitEvalue = max(evalue for div, evalue in minimumHitNameToDivEvalues[hitName])
            maxHitDiv = max(div for div, evalue in minimumHitNameToDivEvalues[hitName])
            ...
            for divEvalue in minimumHitNameToDivEvalues[hitName]:
                ...
                for revHitData in distancesRevHitDataList:
                    if revHitData['revHitEvalue'] < evalue and not revHitData['tooDivergedPred'](div):
                ...
                if queryName in [revHitData['revHitName'] for revHitData in minimumRevHitDatas]:
                    divEvalueToOrthologs[divEvalue].append((hitName, queryName, hitData['distance']))
                    if DEBUG:
                        print 'ortholog\t{0}\t{1}\t{2}'.format(hitName, queryName, hitData['distance'])
