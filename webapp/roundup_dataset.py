...
def splitUniprotIntoGenomes(ds):
    ...
    for path in fastas:
        ...
        with gzip.open(path) as fh:
            for i, (nameline, seq) in enumerate(fasta.readFasta(fh)): # , ignoreParseError=True):
                if i % 1e4 == 0: print i
...
def computePair(ds, pair, workingDir, orthologsPath):
    ...
    with nested.NestedTempDir(dir=roundup_common.LOCAL_DIR) as tmpDir:
        if not isComplete(ds, 'blast', queryGenome, subjectGenome):
            rsd.computeBlastHits(queryFastaPath, subjectIndexPath, forwardHitsPath, maxEvalue, tmpDir, copyToWorking=roundup_common.ROUNDUP_LOCAL)
            markComplete(ds, 'blast', queryGenome, subjectGenome)

        if not isComplete(ds, 'blast', subjectGenome, queryGenome):
            rsd.computeBlastHits(subjectFastaPath, queryIndexPath, reverseHitsPath, maxEvalue, tmpDir, copyToWorking=roundup_common.ROUNDUP_LOCAL)
