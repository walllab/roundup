...
If an actor dies mysteriously, the read lock will timeout and another actor will be able to read the message.

goal: no dependency on a specific RDBMS.  
goal: atomic, process-level concurrency-safe.
warning: no thread-safety guarantees.
future: replace with a real message queuing server.  e.g. RabbitMQ, Amazon SQS.
inspiration: Amazon SQS.
...
DEFAULT_LOCK_TIMEOUT = 24 * 3600 # 1 day


class EmptyQueueError(Exception):
    '''
    Exception used to indicate that a queue has no messages (currently.)
    '''
    pass
...
class MessageQueue(object):
    def __init__(self, queue, manager, timeout=DEFAULT_LOCK_TIMEOUT, drop=False, create=False):
        ...
        timeout: It is important that timeout be longer than it takes a consumer to process a message and delete it or else the message might be read and processed twice.
          Used to recover a message when a consumer dies while processing a message and before deleting it.
        '''
