...
function echoSyncLocalResult($fullyQualifiedFuncName, $keywords, $resultType, $templateType=NULL, $otherUrlParams=NULL) {
  ...
  $resultFilename = getResultFilenameFromId($resultId);
...
function getResultFilenameFromId($resultId) {
  ...
  if ($exitcode) {
    logError('getResultFilenameFromId(): exit code encountered when running roundup_common.makeNestedPath(). keywords='.serialize($keywords));
