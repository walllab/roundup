...
###################################
# PACKAGE AND DEPLOY RSD STANDALONE
###################################

cd ~/work/roundup

# update code
cp -pr rsd_standalone_package rsd_standalone
cp webapp/rsd.py webapp/fasta.py webapp/nested.py webapp/util.py webapp/codeml.ctl webapp/jones.dat rsd_standalone/
chmod 775 rsd_standalone/rsd.py

# clear any backup and .pyc files
find rsd_standalone -type f -name "*~" -print0 -or -name "*\\.pyc" -print0 | xargs -0 rm
...
rm -rf rsd_standalone.tar.gz rsd_standalone/
