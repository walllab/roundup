...
def local():
    ...
    config.group = 'staff' # works for td23 on Mac OS X
...
def perms():
    require('configured')
    # Enforce group-friendly perms: group write, setgid, group owner.
    fix_group_perms(config.deploy_dir, group=config.group)
